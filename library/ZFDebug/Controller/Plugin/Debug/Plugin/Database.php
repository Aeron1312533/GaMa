    ...
    implements ZFDebug_Controller_Plugin_Debug_Plugin_Interface
{
    ...
    public function getPanel()
    {
        ...
        $html .= ' (Maximum ' . $this->_max_rows . ' rows displayed in result set)</h4>';
        ...
        foreach ($this->_db as $name => $adapter) {
            if ($profiles = $adapter->getProfiler()->getQueryProfiles()) {
                ...
                foreach ($profiles as $profile) {
                    ...
                    if ($profile->getQueryType() == Zend_Db_Profiler::SELECT) {
                       $rows = $adapter->fetchAll($profile->getQuery(), NULL, Zend_Db::FETCH_ASSOC);
                       
                       $queries .= "<tr><td style='color: green; text-align:right;padding-right:2em;' nowrap>(" .count($rows) . " rows)</td>"
                               . "<td><table style='border: 1px solid white'>";
                       
                       if (!empty($rows)) {
                            $queries .= "<tr>";
                            foreach ($rows[0] as $column => $value) {
                                $queries .= "<th style='border: 1px solid white; padding-left:2em;padding-right:2em;' nowrap>" . $column . '</th>';
                            }
                            $queries .= "</tr>";
                       }
                       
                       foreach ($rows as $row) {
                           if ($current_rows >= $this->_max_rows) {
                               break;
                           }
                           ...
                           foreach ($row as $value) {
                               $queries .= "<td style='border: 1px solid white; color:#7F7F7F;padding-left:2em;padding-right:2em;' nowrap>" . $value . '</td>';
                           }
