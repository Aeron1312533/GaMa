...
class ZFDebug_Controller_Plugin_Debug extends Zend_Controller_Plugin_Abstract
{
    ...
    {
                ZFDebug_Dumper::dump('blablabla', 'moj vypis');
        if (isset($options)) {
         ...
         * Create dump panel
         */
        $debug_vars = new ZFDebug_Controller_Plugin_Debug_Plugin_Text();
        $debug_vars->setPanel($this->_getDumpPanel())
                   ->setTab($this->_getDumpTab())
                   ->setIdentifier('debug_vars');
        
        $this->registerPlugin($debug_vars);
    ...
    protected function _getDumpPanel() {
        $result = '<h4>Dumped variables:</h4>';  
        
        if (Zend_Registry::isRegistered(ZFDebug_Dumper::_index)) {
            $dumper = Zend_Registry::get(ZFDebug_Dumper::_index);
            
            foreach ($dumper as $row) {
                $result .= '<h4 style="color: green;">' . $row['label'] . '</h4>';
    ...
    protected function _getDumpTab() {
        $tabName = 'Dump';
        $count = 0;
